...
nob.anammox <- read.table("~/Box/PhD/Research/Lake-Tanganyika/1_Code/NOB-Anammox/NOB_Annamox_R.tsv", sep="\t", header=TRUE)

library(tidyverse)
nob.anammox.gathered <- nob.anammox %>% gather(key="MAG", value="Coverage",c(2:61))

# Add a column with guild indo:
guild.info <- read.table("~/Box/PhD/Research/Lake-Tanganyika/1_Code/NOB-Anammox/key_taxonomy.tsv", sep="\t", header=TRUE)
